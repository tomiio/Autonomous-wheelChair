  ...
  const hardware_interface::HardwareInfo & info)
{
  ...
  node_ = rclcpp::Node::make_shared("diffdrive_arduino_hardware_node");

  // Initialize ultrasonic publishers
  ultrasonic_left_pub_ = node_->create_publisher<sensor_msgs::msg::Range>("ultrasonic_left", 10);
  ultrasonic_right_pub_ = node_->create_publisher<sensor_msgs::msg::Range>("ultrasonic_right", 10);
...
std::vector<hardware_interface::StateInterface> DiffDriveArduinoHardware::export_state_interfaces()
{
  ...
  // Ultrasonic sensor state interfaces
  state_interfaces.emplace_back(hardware_interface::StateInterface("ultrasonic_left", "distance", &ultrasonic_left_));
  state_interfaces.emplace_back(hardware_interface::StateInterface("ultrasonic_right", "distance", &ultrasonic_right_));
  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & period)
{
  ...
  comms_.read_ultrasonic_values(ultrasonic_left_, ultrasonic_right_);

  // Publish ultrasonic data
  sensor_msgs::msg::Range left_range_msg;
  left_range_msg.header.stamp = node_->now();
  left_range_msg.header.frame_id = "ultrasonic_left";  // Change to appropriate frame
  left_range_msg.radiation_type = sensor_msgs::msg::Range::ULTRASOUND;
  left_range_msg.field_of_view = 0.3;  // Adjust as needed
  left_range_msg.min_range = 0.1;
  left_range_msg.max_range = 2.0;
  left_range_msg.range = ultrasonic_left_;

  ultrasonic_left_pub_->publish(left_range_msg);

  sensor_msgs::msg::Range right_range_msg;
  right_range_msg.header.stamp = node_->now();
  right_range_msg.header.frame_id = "ultrasonic_right";  // Change to appropriate frame
  right_range_msg.radiation_type = sensor_msgs::msg::Range::ULTRASOUND;
  right_range_msg.field_of_view = 0.3;  // Adjust as needed
  right_range_msg.min_range = 0.1;
  right_range_msg.max_range = 2.0;
  right_range_msg.range = ultrasonic_right_;

  ultrasonic_right_pub_->publish(right_range_msg);
  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  ...
  std::cout << "cmd left: " << wheel_l_.cmd << " cmd right: " << wheel_r_.cmd << std::endl;
